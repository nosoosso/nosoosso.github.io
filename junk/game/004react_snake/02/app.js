...
/***/ function(module, exports, __webpack_require__) {
	...
	window.onkeydown = function (e) {
	    if (e.keyCode === 37) {
	        Action.pressKey('left');
	    } else if (e.keyCode === 38) {
	        Action.pressKey('up');
	    } else if (e.keyCode === 39) {
	        Action.pressKey('right');
	    } else if (e.keyCode === 40) {
	        Action.pressKey('down');
	    }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SnakeGame = React.createClass({
	    ...
	    mixins: [Reflux.connect(GameStore, 'gameStore'), Reflux.connect(TimeStore, 'timeStore')],
	    render: function render() {
	        ...
	        return React.createElement(
	            ...
	            React.createElement(Board, { board: this.state.gameStore.board })
	        );
	    }
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var GameStore = Reflux.createStore({
	    ...
	    onPressKey: function onPressKey(direction) {
	        this.data.snake.direction = direction;
	    },
	    init: function init() {
	        this.listenTo(TimeStore, this.updateSnake);
	        // set initial position
	        this.data.board.snake = List.of(List.of(co.boardSize.x / 2, co.boardSize.y / 2));
	        // put item
	        this.data.board.item = List.of(Math.floor(Math.random() * (co.boardSize.x - 1)), Math.floor(Math.random() * (co.boardSize.y - 1)));
	    },
	    updateSnake: function updateSnake() {
	        var currentPos = undefined;
	        var nextPos = undefined;
	        switch (this.data.snake.direction) {
	            case 'right':
	                currentPos = this.data.board.snake.first();
	                nextPos = List.of(currentPos.get(0) + 1, currentPos.get(1));
	                break;
	            case 'left':
	                currentPos = this.data.board.snake.first();
	                nextPos = List.of(currentPos.get(0) - 1, currentPos.get(1));
	            ...
	            case 'up':
	                currentPos = this.data.board.snake.first();
	                nextPos = List.of(currentPos.get(0), currentPos.get(1) - 1);
	                break;
	            case 'down':
	                currentPos = this.data.board.snake.first();
	                nextPos = List.of(currentPos.get(0), currentPos.get(1) + 1);
	        ...
	        this.data.board.snake = this.data.board.snake.unshift(nextPos);
	
	        var snakeHead = this.data.board.snake.first();
	        var item = this.data.board.item;
	
	        // get item
	        if (snakeHead.get(0) === item.get(0) && snakeHead.get(1) === item.get(1)) {
	            this.data.board.item = List.of(Math.floor(Math.random() * (co.boardSize.x - 1)), Math.floor(Math.random() * (co.boardSize.y - 1)));
	        } else {
	            this.data.board.snake = this.data.board.snake.pop();
	        }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var actions = Reflux.createActions(['tick', 'pressKey']);
	...
	var Board = React.createClass({
	    ...
	    getRows: function getRows() {
	        ...
	        for (var i = 0; i < co.boardSize.y; i++) {
	            rows = rows.push(React.createElement(Row, { board: this.props.board, key: i, y: i }));
	        }
	        return rows;
	    },
	    render: function render() {
	...
	var Row = React.createClass({
	    ...
	    getCells: function getCells() {
	        ...
	        for (var i = 0; i < co.boardSize.x; i++) {
	            var onSnake = false;
	            var onItem = false;
	            //TODO 全てのセルを走査するので重そう
	            if (this.props.board.snake.includes(List.of(i, this.props.y))) {
	                onSnake = true;
	            } else if (this.props.board.item.get(0) === i && this.props.board.item.get(1) === this.props.y) {
	                onItem = true;
	            }
	            cells = cells.push(React.createElement(Cell, { board: this.props.board, key: i, onSnake: onSnake, onItem: onItem }));
	        }
	        return cells;
	    },
	    render: function render() {
